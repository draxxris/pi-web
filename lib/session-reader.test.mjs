...
function j0k3rMarkerEntry(data = {}) {
  return {
    type: "custom",
    customType: "pi-subagents-j0k3r:subagent-session",
    id: "marker",
    parentId: null,
    timestamp: "2026-01-01T00:00:00.000Z",
    data: { version: 1, agent: "pawn", taskId: "subtask_pawn_1", ...data },
  };
}

test("detects the j0k3r nested-session marker in entries", () => {
  assert.equal(hasJ0k3rMarker([userEntry("u1", null, "hi")]), false);
  assert.equal(hasJ0k3rMarker([j0k3rMarkerEntry()]), true);
  assert.equal(
    getJ0k3rMarkerData([j0k3rMarkerEntry({ parentSessionPath: "/tmp/parent.jsonl" })])?.parentSessionPath,
    "/tmp/parent.jsonl",
  );
  assert.equal(getJ0k3rMarkerData([userEntry("u1", null, "hi")]), null);
});

test("readJ0k3rRun resolves display metadata without pi-web subagent schemas", () => {
  const entries = [j0k3rMarkerEntry()];
  const run = readJ0k3rRun(entries, "child", "/tmp/child.jsonl", "parent-id", undefined, true);
  assert.deepEqual(run, {
    ...
    parentToolCallId: "",
    profile: "pawn",
    description: "pawn · subtask_pawn_1",
    task: "subtask_pawn_1",
  ...
  const viaMarkerPath = readJ0k3rRun(entries, "child", "/tmp/child.jsonl", undefined, "marker-parent");
  assert.equal(viaMarkerPath?.parentSessionId, "marker-parent");
  assert.equal(viaMarkerPath?.status, "interrupted");

  assert.equal(readJ0k3rRun(entries, "child", "/tmp/child.jsonl"), null);
  assert.equal(readJ0k3rRun([userEntry("u1", null, "hi")], "child", "/tmp/child.jsonl", "parent-id"), null);
});

test("recognizes the persisted j0k3r sub-agent marker", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-subagent-marker-"));
  const markedPath = join(dir, "marked.jsonl");
  const ordinaryPath = join(dir, "ordinary.jsonl");
  writeFileSync(markedPath, [
    JSON.stringify({ type: "session", id: "marked", timestamp: new Date().toISOString(), cwd: dir }),
    JSON.stringify({
      type: "custom",
      customType: "pi-subagents-j0k3r:subagent-session",
      id: "marker",
  ...
  writeFileSync(ordinaryPath, JSON.stringify({ type: "session", id: "ordinary", timestamp: new Date().toISOString(), cwd: dir }));

  try {
    assert.equal(isSubagentSessionFile(markedPath), true);
    assert.equal(isSubagentSessionFile(ordinaryPath), false);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
