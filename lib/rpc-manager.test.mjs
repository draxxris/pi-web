...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(startupSource, /preferPiWebSubagentExtension\((?:withoutPiMiseExtension\(base\)|base)\)/);
});
...
test("running-session ids include live nested sub-agent sessions", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const runningSource = source.slice(source.indexOf("export function getRunningRpcSessionIds"));

  assert.match(runningSource, /getLiveSubagentSessionIds\(\)/);
  assert.match(
    source,
...
test("startRpcSession adopts a live nested sub-agent session handle", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(startSource, /adoptLiveSubagentSession\(sessionId, existing\)/);
  assert.match(source, /const inner = getLiveSubagentSession\(sessionId\)/);
  assert.match(source, /new AgentSessionWrapper\(inner, \{[\s\S]*?suppressCompletionNotifications: true,[\s\S]*?ownsInnerSession: false,/);
});

test("running-session API marks only unadoptable nested runs external", async () => {
  const source = await readFile(new URL("../app/api/agent/running/route.ts", import.meta.url), "utf8");
  assert.match(source, /externalRunningSessionIds/);
  assert.match(source, /getUnmanagedSubagentSessionIds\(\)/);
});
